...
class Diya1 extends ComicSource {
  name = "蒂亚漫画";
  key = "legacy_diya1";
  version = "0.1.0";
  minAppVersion = "1.6.0";
  url = "";
  get baseUrl() { return "https://m.diya1.com"; }
  ...
  comic = {
    loadInfo: async (id) => {
      ...
      return new ComicDetails({ title: this.text(doc.querySelector("h1")) || this.text(doc.querySelector(".title")), cover: this.absolute(this.attr(doc.querySelector("#Cover img, .pic img, img"), "src")), description: this.text(doc.querySelector("#full-des, #simple-des, .txtDesc")), tags, chapters: this.chaptersOf(doc) });
    },
