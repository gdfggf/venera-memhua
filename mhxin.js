...
class Mhxin extends ComicSource {
  name = "漫画芯";
  key = "legacy_mhxin";
  version = "0.1.0";
  minAppVersion = "1.6.0";
  url = "";
  get baseUrl() { return "https://m.mhxin.com"; }
  ...
  async raw(url, options) { const r = options ? await Network.post(url, options.headers || this.headers, options.body) : await Network.get(url, this.headers); if (r.status !== 200) throw `请求失败：${r.status}`; return r.body; }
  parseCards(doc, base) {
    const comics = [];
    for (const card of [...doc.querySelectorAll(".list-comic"), ...doc.querySelectorAll(".itemBox")]) {
      const link = card.querySelector("a");
      const image = card.querySelector("img");
      const id = this.absolute(this.attr(link, "href"), base);
      const title = this.text(card.querySelector(".txtA")) || this.text(card.querySelector(".title")) || this.text(link);
      if (id && title) comics.push(new Comic({ id, title, cover: this.attr(image, "src") || "", subTitle: this.text(card.querySelector(".info, .date")) }));
    }
    return comics;
  }
  chaptersOf(doc) {
  ...
  search = {
    load: async (keyword, options, page) => {
      const body = await this.raw(`https://api.mhxin.com/comic/search/?page=${page}`, { headers: { "Content-Type": "application/x-www-form-urlencoded", "Referer": this.baseUrl + "/" }, body: `keywords=${encodeURIComponent(keyword)}` });
      let comics = [];
      try {
        const json = JSON.parse(body);
        const items = (json && (json.items || json.data)) || [];
        for (const it of items) {
          const uri = it.uri || (it.id ? "/comic/" + it.id + "/" : "");
          const title = it.name || it.title;
          if (uri && title) comics.push(new Comic({ id: this.absolute(uri), title, cover: it.cover || it.coverUrl || "", subTitle: it.last_chapter_name || "" }));
        }
      } catch (_) {
        comics = this.parseCards(new HtmlDocument(body), this.baseUrl);
      }
  ...
  comic = {
    ...
    loadEp: async (comicId, epId) => {
      ...
      while (url && guard < 20) {
        ...
        for (const img of doc.querySelectorAll(".image-content img, .erPag mip-img, .erPag img, .comicpage img")) {
