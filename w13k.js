...
class W13k extends ComicSource {
  name = "13漫画网";
  key = "legacy_w13k";
  version = "0.1.0";
  minAppVersion = "1.6.0";
  url = "";
  get baseUrl() { return "https://m.w13k.com"; }
  ...
  parseCards(doc, base) {
    const comics = [];
    for (const card of [...doc.querySelectorAll(".itemBox"), ...doc.querySelectorAll(".list-comic"), ...doc.querySelectorAll(".col_3_1 li")]) {
  ...
  comic = {
    ...
    loadEp: async (comicId, epId) => {
      ...
      while (url && guard < 20) {
        ...
        for (const img of doc.querySelectorAll(".erPag mip-img, .erPag img, .comicpage mip-img, .comicpage img, #chapter-image img, .image-content img")) {
