...
class M90mh extends ComicSource {
  name = "90漫画";
  key = "legacy_90mh";
  version = "0.1.0";
  minAppVersion = "1.6.0";
  url = "";
  get baseUrl() { return "http://m.90mh.com"; }
  ...
  parseCards(doc, base) {
    ...
    for (const card of [...doc.querySelectorAll(".itemBox"), ...doc.querySelectorAll(".list-comic")]) {
      ...
      if (id && title) comics.push(new Comic({ id, title, cover: this.attr(image, "src") || "" }));
    }
    return comics;
  }
  chaptersOf(doc) {
  ...
  search = { load: async (keyword, options, page) => { const doc = await this.html(`${this.baseUrl}/search/?keywords=${encodeURIComponent(keyword)}`); const cs = this.parseCards(doc, this.baseUrl); if (!cs.length) throw "没有解析到搜索结果（站点页面结构可能已更新）"; return { comics: cs, maxPage: 1 }; }, enableTagsSuggestions: false };
  comic = {
    loadInfo: async (id) => {
      const doc = await this.html(id);
      const tags = { "作者": [], "类型": [] };
      for (const dd of doc.querySelectorAll("dl dd")) { const t = this.text(dd); if (t && t.length < 30) tags["作者"].push(t); }
    ...
    loadEp: async (comicId, epId) => {
      ...
      while (url && guard < 20) {
        ...
        for (const img of doc.querySelectorAll("#chapter-image img, .erPag mip-img, .erPag img, .comicpage img")) {
